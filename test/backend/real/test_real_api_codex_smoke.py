"""Real API-path Codex smoke for Phase 6.
...
async def main() -> int:
    if not shutil.which("codex"):
        print(json.dumps({"ok": False, "error": "codex not found"}, ensure_ascii=False))
        return 1

    workspace = Path(tempfile.mkdtemp(prefix="agenthub-api-codex-"))
    project_id = str(uuid.uuid4())
    agent_id = str(uuid.uuid4())
    session_id = str(uuid.uuid4())
    prompt = (
        "Create a file named api-codex-smoke.txt containing exactly "
        "AGENTHUB_API_CODEX_WORKSPACE"
    )
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            db.add(AgentConfig(
                ...
                description="Real Codex smoke",
                system_prompt="",
                agent_type="cli_wrapper",
                cli_tool="codex",
                executable="codex",
                init_args=json.dumps(DEFAULT_CLI_AGENTS["codex"]["init_args"]),
                env_vars="{}",
            ))
            db.add(Session(
                id=session_id,
                title="real-api-codex-smoke",

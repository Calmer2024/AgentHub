...
def _phase10_cli() -> Path:
    script = BACKEND_ROOT / ".test-bin" / "phase10_cloud_cli.py"
...
def _cloud_group_worker_cli() -> Path:
    script = BACKEND_ROOT / ".test-bin" / "phase10_cloud_group_worker.py"
    script.parent.mkdir(parents=True, exist_ok=True)
    script.write_text(
        "import os, sys\n"
        "data = os.read(sys.stdin.fileno(), 65536).decode('utf-8', errors='replace')\n"
...
def _cloud_group_orchestrator_cli() -> Path:
    script = BACKEND_ROOT / ".test-bin" / "phase10_cloud_group_orchestrator.py"
    script.parent.mkdir(parents=True, exist_ok=True)
    script.write_text(
...
def _cloud_group_plan_worker_cli() -> Path:
    script = BACKEND_ROOT / ".test-bin" / "phase10_cloud_group_plan_worker.py"
    script.parent.mkdir(parents=True, exist_ok=True)
    script.write_text(
        "import os, sys\n"
        "data = os.read(sys.stdin.fileno(), 65536).decode('utf-8', errors='replace')\n"
...
def _cloud_group_plan_orchestrator_cli(worker: dict) -> Path:
    script = BACKEND_ROOT / ".test-bin" / "phase10_cloud_group_plan_orchestrator.py"
    script.parent.mkdir(parents=True, exist_ok=True)
    script.write_text(
        "import json, os, sys\n"
        "data = os.read(sys.stdin.fileno(), 65536).decode('utf-8', errors='replace')\n"

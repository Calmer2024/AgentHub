...
function mergeHydratedMessages(
  ...
  const hasPreservedLiveMessage = current.some((message) => (
    message.sessionId === sessionId && shouldPreserveLiveMessage(message, runs)
  ));
  const hasActiveRuntime = runtime.isStreaming
    || runs.some((run) => ACTIVE_RUN_STATUSES.has(run.status))
    || hasPreservedLiveMessage;
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  ensureAgentMessage: (input) =>
    set((s) => {
      const current = s.messagesBySession[input.sessionId]
        ?? (s.currentSessionId === input.sessionId ? s.messages : []);
      if (current.some((message) => message.id === input.id)) return {};
      ...
      return {
        messagesBySession: { ...s.messagesBySession, [input.sessionId]: messages },
        ...applyCurrentSession(s, input.sessionId, { messages }),
      };
    }),

...
export function OrchestratorPlanPanel({ plan, rawJson }: Props) {
  ...
  return (
    <div className="space-y-3">
      <section className="agenthub-card rounded-lg border p-3 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="agenthub-muted mt-1 text-xs">
      ...
      {plan.visualization?.mermaid && (
        <section className="agenthub-card rounded-lg border p-3 shadow-sm">
          <div className="agenthub-strong mb-2 flex items-center gap-2 text-sm font-semibold">
            <GitBranch size={15} className="text-[color:var(--ah-info)]" />
            <span>DAG 预览</span>
          </div>
      ...
      <section className="agenthub-card rounded-lg border shadow-sm">
        <button
          type="button"
          onClick={() => setShowRaw((value) => !value)}
          className="agenthub-muted flex w-full items-center justify-between gap-2 px-3 py-2 text-left text-xs font-semibold hover:bg-[color:var(--ah-accent-soft)]"
        >
          <span>原始 JSON</span>
          {showRaw ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
        </button>
        {showRaw && (
          <pre className="agenthub-code-block max-h-80 overflow-auto border-t p-3 text-[11px] leading-5">
            {rawJson}
          </pre>
        )}
      </section>
...
function StatusBadge({ ok }: { ok: boolean }) {
  return (
    <span className={`inline-flex shrink-0 items-center gap-1 rounded-md border px-2 py-1 text-[11px] font-semibold ${
      ok ? "agenthub-status-success" : "agenthub-status-error"
    }`}>
...
function ValidationSummary({ validation }: { validation?: OrchestratorValidation }) {
  if (!validation || (validation.errors.length === 0 && validation.warnings.length === 0)) {
    return <p className="mt-3 text-xs text-[color:var(--ah-success)]">结构校验通过，无 warning。</p>;
  }
  return (
...
function TaskTimeline({ plan }: { plan: OrchestratorPlan }) {
  ...
  return (
    <section className="space-y-2">
      {phases.map((phase) => (
        <div key={phase.index} className="agenthub-card rounded-lg border p-3 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-2">
            <div className="agenthub-strong flex items-center gap-2 text-sm font-semibold">
              <ListChecks size={15} className="text-[color:var(--ah-info)]" />
              <span>Phase {phase.index}</span>
            </div>
            <span className="agenthub-status rounded px-2 py-1 text-[11px] font-medium">
              {phase.tasks.length > 1 ? "并行" : "串行"}
            </span>
          </div>
          <div className="space-y-2">
            {phase.tasks.map((task) => (
              <article key={task.task_id} className="agenthub-soft rounded-md border p-3">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="min-w-0">
                    <h4 className="agenthub-strong text-sm font-semibold">{task.task_id} · {task.title}</h4>
...
function InfoLine({ label, value }: { label: string; value: string }) {
  return (
    <p>
      <span className="agenthub-faint font-semibold">{label}：</span>
      <span>{value}</span>
    </p>
  );
}
...
function CompactList({ title, items }: { title: string; items: string[] }) {
  ...
  return (
    <div className="mt-2">
      <p className="agenthub-faint text-[11px] font-semibold">{title}</p>
      <ul className="agenthub-muted mt-1 list-disc space-y-1 pl-4 text-[11px] leading-5">
...
function MermaidPreview({ chart }: { chart: string }) {
    ...
    const render = async () => {
      ...
      try {
        ...
        mermaid.initialize({
          ...
          theme: "base",
          themeVariables: {
            background: styles.getPropertyValue("--ah-card-bg").trim() || "transparent",
            primaryColor: styles.getPropertyValue("--ah-card-soft").trim() || "#eef2ff",
            primaryTextColor: styles.getPropertyValue("--ah-text-strong").trim() || "#0f172a",
            primaryBorderColor: styles.getPropertyValue("--ah-border-strong").trim() || "#4f46e5",
            lineColor: styles.getPropertyValue("--ah-muted").trim() || "#64748b",
  ...
  if (error) return <p className="text-xs text-[color:var(--ah-danger)]">{error}</p>;
  return <div ref={containerRef} className="agenthub-soft max-h-96 overflow-auto rounded-md border p-2 [&_svg]:mx-auto [&_svg]:h-auto [&_svg]:max-w-none" />;
}

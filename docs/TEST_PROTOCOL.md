...
- **P2 双运行时门禁**：Phase 9 起，大型 Phase 验收和发布候选必须证明 SaaS 云端能力以真实可运行切片递增，同时 P1 本机版主路径不回归。小修按影响面选择相关测试。
...
```
按改动影响面选择:
  1. 纯文档/配置说明: 文档阅读检查或链接检查
  2. 后端小修: 直接相关 pytest 文件/用例
  3. 前端小修: 直接相关 vitest 文件/用例；涉及类型契约时加 npx tsc --noEmit
  4. API/前端交互/运行时改动: 相关 API/组件测试 + 必要真实服务验收
  5. 大型 Phase、发布候选、跨模块架构、Auth/租户/Secret/运行时/部署: 执行完整回归组合
```
...
Phase 完成前:
  ...
  6. 涉及 API、前端交互、运行时、部署、代理或端到端用户路径时，启动当前代码服务执行真实服务验收
  7. 手动验证清单 (Spec 第 4 节)
  8. 全部通过 → Phase 标记为 Completed
```
...
涉及 API、前端交互、运行时、部署、代理或端到端用户路径的改动必须执行；纯文档、纯类型、纯单元逻辑或不影响服务行为的小修可只跑对应自动化测试或文档检查。
...
3. **验证新进程**：
   ...
   - 前端 `/api` 代理能访问当前后端。
   - 本轮改动的关键路径在真实服务上通过。
4. **交付地址**：执行真实服务验收时，最终回复必须包含相关服务 URL；若端口不是默认值，必须明确说明。

需要真实服务验收却未执行上述步骤，或旧服务仍在提供旧代码，视为验收未完成。
...
| 门禁 | 必测路径 | 失败处理 |
|------|---------|---------|
| P1 local 回归 | 大型 Phase/发布候选需覆盖本机 Project 创建、本机私聊/群聊、真实 CLI runtime、Artifact Card、本地 build/preview/export、审批续跑；小修按影响面抽测 | 阻断 Phase 完成，先修复本机回归 |
...
- [ ] 大型 Phase/发布候选：上一 Phase 主路径仍通过（无回归）
- [ ] Phase 9 起大型 Phase/发布候选：P1 local 主路径回归通过，SaaS 新增能力以本 Phase cloud slice 可运行方式通过
- [ ] 涉及服务行为时，旧后端/前端进程已清理，新服务进程已从当前仓库启动
- [ ] 涉及服务行为时，真实服务验收已通过，且最终交付包含相关服务地址
...
| 旧进程仍在服务旧代码 | 单元测试通过，但浏览器/API 访问不到新增接口 | 涉及服务行为的改动必须检查端口监听进程、OpenAPI、前端 `/api` 代理和真实服务关键路径；纯文档或纯单元逻辑改动不需要固定启动服务。 |
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-06-10 | v2.6 | 取消每次小修固定全量回归组合，改为按影响面选择相关测试；大型 Phase、发布候选和高风险跨模块改动才跑完整回归 |
| 2026-06-10 | v2.5 | 将每轮固定服务交接调整为按需真实服务验收：涉及服务行为时仍强制，纯文档/纯单元逻辑不再要求启动前后端 |

...
Phase 6 回答这三个问题。它引入 **Project** 作为顶层组织实体，实现三个 CLI 工具的专属适配器，并把产品概念从“裸 CLI 好友”升级为 **Agent Profile = System Prompt + Rules + Toolset + Context Policy + Runtime Config + Engine**，最终打通从 Agent 输出到 Artifact Card 的完整链路。
...
创建 Project + 绑定 workspace 目录
  ...
  → 路由到 Agent Profile
  → 解析 System Prompt + Rules + Toolset + Context Policy + Engine Runtime
...
| CLI Wrapper 是 AgentHub 唯一的 Engine 执行模式；用户可见 Agent = System Prompt + Rules + Toolset + Context Policy + Runtime Config + Engine | ADR-0011 |
...
- AC-PROFILE-01: `GET /api/agents` 返回 `systemPrompt / rules / toolset / avatar / contextPolicy`
- AC-PROFILE-02: 添加 Agent 面板内置 7 个专家模板，选择后预填 System Prompt、Rules、Engine、工具集、头像、Context Policy
- AC-PROFILE-03: Prompt Assembly 注入 System Prompt、Rules、工具集摘要与本机 Skill 内容
- AC-PROFILE-04: AgentSelector 优先按工具集和内置角色标签匹配，再 fallback 到 name/description/systemPrompt
- AC-PROFILE-05: 调度器被建模为内置 Agent 模板，不再作为孤立概念
- AC-PROFILE-06: 默认好友列表不自动包含 7 个专家模板，旧自动 seed 模板会归档

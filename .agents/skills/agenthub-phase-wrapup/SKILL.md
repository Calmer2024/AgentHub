...
- 文档层级是否清晰：入口层(Codex/CONTEXT) → 决策层(ADR) → 规格层(Spec) → 协议层(Protocol) → 记录层(Dev Log)
- 新内容是否归入了已有文档？（先考虑补充现有文档，再考虑新建）
- 跨文档引用是否用链接而非复制？（见 AGENTS.md Documentation Rules）
...
- 引用修复：全量文档扫描对象从不存在的 `.Codex/skills/` 修正为当前 `.agents/skills/`，并注明 `.claude/skills/` 为镜像审计对象。

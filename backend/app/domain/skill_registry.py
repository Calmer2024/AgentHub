...
BUILTIN_SKILLS: tuple[SkillDefinition, ...] = (
    ...
    SkillDefinition(
        ...
        prompt=(
            "你具备后端工程能力，专注于 API 设计、数据库模型、服务端逻辑、"
            "数据一致性、错误处理和后端测试。"
        ),
    ),
    SkillDefinition(
        ...
        prompt=(
            "你具备产品管理能力，负责把模糊想法收敛成可执行产品需求：明确目标用户、"
    ...
    SkillDefinition(
        ...
        prompt=(
            "你具备需求分析能力，专注识别隐藏规则、异常流程、角色权限、状态流转、"
    ...
    SkillDefinition(
        ...
        prompt=(
            "你具备 API 设计能力，负责定义稳定清晰的接口契约，包括资源模型、请求响应、"
    ...
    SkillDefinition(
        ...
        prompt=(
            "你具备数据库设计能力，负责实体关系、表结构、字段约束、索引、迁移策略、"
    ...
    SkillDefinition(
        ...
        prompt=(
            "你具备测试工程能力，负责制定测试策略、识别高风险路径、设计正常/异常/权限/"
    ...
    SkillDefinition(
        ...
        prompt=(
            "你具备技术文档能力，负责把产品、架构、接口、测试和交付信息整理成清晰文档。"
    ...
    SkillDefinition(
        ...
        prompt=(
            "你具备 UX 设计能力，负责用户流程、信息架构、页面状态、交互反馈、空状态/"
    ...
    SkillDefinition(
        ...
        prompt=(
            "你具备 AgentHub 调度规划能力。该能力的职责是把用户需求拆解为"
            ...
            "6. expected_outputs 要区分正式产物与内部草稿。用户要求 PRD、架构设计、接口说明、"
            "测试清单等项目文档时，应建议写入项目 `docs/`；内部交接和草稿才留在任务工作包。\n"
            "7. 如果当前上下文没有候选 Agent 列表，可以把 assigned_agent_id 设为 null，"
